...
export function DataTableSortList<TData>({
  ...
  React.useEffect(() => {
    table.options.onSortingChange?.(sorting)
  }, [sorting, table])
    ...
    () =>
      table
        ...
        .filter(
          (column) =>
            column.getCanSort() &&
            !sorting.some((s: ExtendedColumnSort<TData>) => s.id === column.id)
        )
        .map((column) => ({
          id: column.id,
  ...
  function updateSort({
    ...
    updateFunction((prevSorting: ExtendedSortingState<TData>) => {
  ...
  function removeSort(id: string) {
    void setSorting((prevSorting: ExtendedSortingState<TData>) =>
        ...
        >
          ...
          <SortableContent asChild>
            <div className='flex max-h-40 flex-col gap-2 overflow-y-auto p-0.5'>
              {uniqueSorting.map((sort: ExtendedColumnSort<TData>) => {
