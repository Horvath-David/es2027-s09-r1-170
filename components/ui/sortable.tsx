...
import { composeEventHandlers, useComposedRefs } from "@/lib/data-table/composition"
    ...
    (event: DragEndEvent) => {
      ...
      if (over && active.id !== over?.id) {
        const activeIndex = value.findIndex(
          (item: any) => getItemValue(item) === active.id
        )
        const overIndex = value.findIndex(
          (item: any) => getItemValue(item) === over.id
        )
    ...
    >
      <DndContext
        ...
        onDragStart={composeEventHandlers(
          ...
          ({ active }: any) => setActiveId(active.id)
        )}
...
const SortableItem = React.forwardRef<HTMLDivElement, SortableItemProps>(
  (props, forwardedRef) => {
    ...
    const composedRef = useComposedRefs(forwardedRef, (node: any) => {
      if (disabled) return
      setNodeRef(node)
